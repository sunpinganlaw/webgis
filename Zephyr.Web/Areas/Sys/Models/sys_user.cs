...
left join sys_userRoleMap B on B.RoleCode = A.RoleCode and B.UserCode = '{0}'", user);
        ...
        public object ModifyUserPassword(string jsonstr)
        {

            string[] strs = jsonstr.Split(',');
            string UserCode = strs[0];
            string OldPassword = strs[1];
            ...
            if (result == null || String.IsNullOrEmpty(result.UserCode))
            { return new { status = "error", message = "原密码输入错误！" }; }

            var defaultPassword = NewPassword;
            var result_modify = db.Update("sys_user")
                .Column("Password", defaultPassword)
                .Where("UserCode", UserCode)
                .Execute();
            if (result_modify>0)
            {
                { return new { status = "success", message = "修改密码成功！" }; }

            }
            else
            {

                { return new { status = "error", message = "修改密码失败,从再次修改！" }; }
            }
