    ...
    com.dialog({
        ...
        viewModel: function (w) {
            ...
            w.find("#pwd_confirm").click(function () {
                ...
                var message = $('#UserCode').val() + ',' + $('#OldPassword').val() + ',' + $('#NewPassword').val()
                com.ajax({
                    type: 'POST',
                    url: '/api/sys/user/postmodifypassword/' + message,
                    success: function (d) {
                        if (d.status == 'success') {
                            $.messager.alert('系统消息', '密码修改成功');
                            w.dialog('close');
                        } else {
                            $.messager.confirm('系统提示', d.message, function (r) {
                                if (r) {
                                    w.dialog('close');
